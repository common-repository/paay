...
require_once dirname(__FILE__).'/src/PAAY/Auth/Signature.php';
...
add_action('woocommerce_order_status_pending_to_on-hold_notification', 'paay_prevent_order_receipt_notification', 1);
function paay_prevent_order_receipt_notification($order_id) {
    $order = new WC_Order($order_id);
    if ('paay_gateway' == $order->payment_method) {
        $emails = WC_Emails::instance();
        $emails->emails['WC_Email_Customer_Processing_Order']->enabled = 'no';
        return;
    }
}

// add_action('woocommerce_before_checkout_form', 'paay_standalone_hinter');

// function paay_standalone_hinter()
// {
//     $gateway = new Paay_Gateway();
//     $api_url = $gateway->settings['paay_standalone_host'];

//     require_once dirname(__FILE__).'/templates/paay_standalone_hinter.php';
// }
/**
 * Helper methods
 */
...
function paay_api()
{
    ...
    if (null === $api) {
        $api = new Paay_ApiClient(
            ...
            $gateway->settings['paay_3ds_strategy'],
            paay_wc()
        );
    }
